...
public class NormalisedParamScan extends ParamScan {
    ...
    List<IScanIssue> doScan(IHttpRequestResponse baseRequestResponse, IScannerInsertionPoint insertionPoint) {
        ...
        byte[] poisonReq = BulkUtilities.addCacheBuster(insertionPoint.buildRequest(canary.getBytes()), BulkUtilities.generateCanary());
        byte[] victimReq = BulkUtilities.replaceFirst(poisonReq, "kkvjq%61".getBytes(), "kkvjqa".getBytes());
