...
- Tick `external subdomain lookup` to dynamically look up known subdomains using ip.thc.org. Warning: this discloses the top-level private domain that you are targeting. For example, if you target `beta.api.example.com`, The Hacker's Choice will see `example.com` in their server logs. That's why it's not enabled by default.
